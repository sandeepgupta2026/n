...
class Rocket {
    constructor(x) {
        ...
        this.target = window.innerWidth <= 600 ? 90 + Math.random() * 150 : 100 + Math.random() * 200;
...
class Particle {
    constructor(x, y, color) {
        ...
        const speed = window.innerWidth <= 600 ? 1.5 + Math.random() * 4 : 2 + Math.random() * 6;
...
function createExplosion(x, y, color) {
    const count = window.innerWidth <= 600 ? 65 : 120;
...
function launchRockets() {
    ...
    const mobileFireworks = window.innerWidth <= 600;
    const initialRockets = mobileFireworks ? 5 : 10;
    const sidePadding = mobileFireworks ? 38 : 80;

    for (let i = 0; i < initialRockets; i++) {
        const x = initialRockets === 1
            ? canvas.width / 2
            : sidePadding +
              i * ((canvas.width - sidePadding * 2) / (initialRockets - 1));
    ...
    const rocketInterval = setInterval(() => {
        rockets.push(
            new Rocket(
                sidePadding +
                Math.random() * (canvas.width - sidePadding * 2)
            )
        );
    }, mobileFireworks ? 650 : 400);
...
function createDotTextPoints(text) {
    ...
    // IMPORTANT:
    // Use the real viewport size. The old version used a minimum
    // 1000px canvas, which pushed text outside narrow mobile screens.
    const mobile = dW <= 600;
    ...
    if (text === "HAPPY BIRTHDAY" && mobile) {
        // On 360x800 (and other narrow screens), split the message
        // into two centered lines so neither word can leave the screen.
        const fontSize = Math.min(dW * 0.155, 56);

        tctx.font =
            "900 " + fontSize +
            "px Arial Black, Arial, sans-serif";

        tctx.fillText("HAPPY", tw / 2, th * 0.43);
        tctx.fillText("BIRTHDAY", tw / 2, th * 0.55);
    } else {
        let fontSize;

        if (text === "HAPPY BIRTHDAY") {
            fontSize = Math.min(dW * 0.075, 105);
        } else {
            fontSize = mobile
                ? Math.min(dW * 0.25, 96)
    ...
    if (!candidates.length) {
        return Array.from({ length: DOT_TOTAL }, () => ({
            x: dW / 2,
            y: dH / 2
        }));
    }
...
function createFinalDotParticles() {
    const happy = createDotTextPoints("HAPPY BIRTHDAY");
...
function getFinalDotTarget(p, time) {
    ...
    if (t < DOT_NEHA_FORMATION) {
        ...
        return {
            x: dLerp(p.happyX, p.nehaX, progress),
            y: dLerp(p.happyY, p.nehaY, progress)
        };
    }
